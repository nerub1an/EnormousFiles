...
namespace AltiumSort
{
  class Program
  {
      ...
      string fileName, long fileSize, string sortFilePath, ISorter sorter, Stopwatch timer)
    {
      ...
      var mergeService = new MergeService(sortFilePath, chars, sorter, new AltiumComparer());
    ...
    static void Main(string[] args)
    {
      ...
      ISorter sorter = Environment.ProcessorCount == 1
        ? (ISorter)new Sorter(new AltiumComparer())
        : (ISorter)new MultiThreadSorter(new AltiumComparer());
